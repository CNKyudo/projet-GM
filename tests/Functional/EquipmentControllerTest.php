...
final class EquipmentControllerTest extends AbstractWebTestCase
{
    ...
    public function testEditFormPostWithErrorsPreservesBackHrefFromSession(): void
    {
        ...
        // Step 1: open edit from show page → backHref=show is stored in session
        $crawler = $this->client->request(Request::METHOD_GET, $editUrl, [], [], [
            'HTTP_REFERER' => $showUrl,
        ]);
        $this->assertResponseIsSuccessful();
        ...
        // Step 2: submit form with all owners empty → triggers ExactlyOneOwner error
        // The Referer is now the edit page itself (or empty), but session should keep show
    ...
    public function testDifferentEquipmentsHaveIndependentBackHref(): void
    {
        ...
        // Step 1: open edit for gloveA from show page → stores backHref=showA under equipment_edit_back_href_{gloveAId}
        $crawler = $this->client->request(Request::METHOD_GET, $editA, [], [], [
            'HTTP_REFERER' => $showA,
        ]);
        $this->assertSame($showA, $crawler->selectLink('Retour')->attr('href'));
        $this->assertSame($showA, $crawler->selectLink('Annuler')->attr('href'));

        // Step 2: open edit for gloveB from show page → stores backHref=showB under equipment_edit_back_href_{gloveBId}
        $crawler = $this->client->request(Request::METHOD_GET, $editB, [], [], [
            'HTTP_REFERER' => $showB,
        ]);
        $this->assertSame($showB, $crawler->selectLink('Retour')->attr('href'));
        $this->assertSame($showB, $crawler->selectLink('Annuler')->attr('href'));

        // Step 3: POST on gloveA edit with errors → should read session key for gloveA, NOT gloveB
        ...
        $this->assertSame(
            $showA,
            $crawler->selectLink('Retour')->attr('href'),
            'Back button should link to gloveA show page, not gloveB'
        );
        $this->assertSame(
            $showA,
            $crawler->selectLink('Annuler')->attr('href'),
            'Back button should link to gloveA show page, not gloveB'
        );
    }
